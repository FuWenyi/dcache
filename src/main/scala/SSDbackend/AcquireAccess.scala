...
class AcquireAccess(edge: TLEdgeOut)(implicit p: Parameters) {
  ...
  val (grant_first, _, grant_done, grant_count) = edge.count(io.mem_grantD)

  val isGrant = io.mem_releaseAck.bits.opcode === TLMessages.Grant || io.mem_releaseAck.bits.opcode === TLMessages.GrantData
  ...
  val dataRefill = MaskData(io.mem_grantD.data, req.wdata, Mux(addr.wordIndex === grant_count, 0.U(DataBits.W), wordMask))
  ...
  val package = LookupTree(state, List(
      s_get -> get
      s_acqB -> acquireBlock, 
      s_acqP -> acquirePerm, 
      s_put -> putFullData
  ...
  io.mem_getPutAcquire.bits := Mux(state === s_put, Mux(isFullPut, putFullData, putPartialData), package)
